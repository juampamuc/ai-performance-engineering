...
Treat `cluster_gemm_tcgen05` as a supplementary control benchmark with a local contract. It is still useful for tcgen05 profiling and regression tracking, but the lab's canonical speed claim stays on `cluster_gemm`.
...
- Standalone `moe_hybrid_ep_entrypoint` `report.json` artifacts now discard the same warmup steps as the harness before summarizing measured iterations, so report totals reflect the benchmark's steady-state contract instead of a cold first step.
...
- `cluster_gemm_tcgen05` is a supplementary control benchmark with a local contract; use `cluster_gemm` when you want the lab's canonical cluster-GEMM speed claim.
