"""Optimized schedule comparison: GPipe vs DualPipeV (DualPipeV with tuned micro-batches)."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Optimized DualPipeV schedule comparison.")
...
def main():
    ...
    require_min_gpus(2, script_name="optimized_pipeline_gpipe_to_dualpipev_multigpu.py")
    ...
    for step in range(args.steps):
        ...
        if step % args.log_every == 0:
            print(
                f"[optimized-dualpipev-compare] step {step + 1}/{args.steps} "
    ...
    print(
        f"[optimized-dualpipev-compare] done in {elapsed:.2f}s | avg_loss={avg_loss:.4f} | "
...
def get_benchmark():
    return TorchrunScriptBenchmark(
        ...
        name="optimized_pipeline_gpipe_to_dualpipev_multigpu_2stages",
    )
