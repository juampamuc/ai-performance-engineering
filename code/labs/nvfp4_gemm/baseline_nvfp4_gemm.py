"""Python harness wrapper for baseline_nvfp4_gemm.cu."""
...
class BaselineNvfp4GemmBenchmark(CudaBinaryBenchmark):
    """Wraps the baseline NVFP4 GEMM CUDA binary."""

    def __init__(self) -> None:
        lab_dir = Path(__file__).parent
        super().__init__(
            chapter_dir=lab_dir,
            binary_name="baseline_nvfp4_gemm",
            friendly_name="Baseline NVFP4 GEMM",
            iterations=1,
            warmup=5,
            timeout_seconds=240,
            workload_params={
...
def get_benchmark() -> BaseBenchmark:
    return BaselineNvfp4GemmBenchmark()
