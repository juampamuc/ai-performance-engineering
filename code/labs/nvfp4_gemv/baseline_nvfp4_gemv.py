"""Benchmark harness wrapper for NVFP4 GEMV baseline submission."""
...
class BaselineNvfp4GemvBenchmark(VerificationPayloadMixin, BaseBenchmark):
    """Runs the baseline NVFP4 GEMV submission through the benchmark harness."""
    ...
    def setup(self) -> None:
        ...
        reference_mod = load_reference_module(
            ...
            module_name="nvfp4_gemv_reference_for_baseline",
            utils_module=utils_mod,
        )
        baseline_mod = load_submission_module(
            LAB_DIR / "baseline_submission.py",
            module_name="nvfp4_gemv_baseline_submission",
        ...
        if not callable(kernel_fn):
            raise RuntimeError("baseline_submission.py must expose custom_kernel(data)")
