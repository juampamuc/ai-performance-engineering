...
| `decode_multigpu_demo.py` | Multi-GPU demo/tool to stress NVLink-C2C bandwidth on available GPUs (torchrun required). |
...
To run the multi-GPU NVLink-C2C demo, use the demos runner:
```bash
...
- Multi-GPU demo exercises NVLink-C2C without graph capture failures when launched via `torchrun`.
