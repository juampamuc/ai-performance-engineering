#!/usr/bin/env python3
"""Baseline TP-only benchmark that all-gathers sequence activations each layer."""
...
def main() -> None:
    args = _parse_args()
    require_min_gpus(2, script_name="baseline_sequence_parallel_multigpu.py")
    ...
    run_sequence_parallel(config=config, iters=args.iters, warmup=args.warmup, sequence_parallel=False)


class BaselineSequenceParallelMultigpuBenchmark(VerificationPayloadMixin, BaseBenchmark):
    """Harness entry that launches the TP-only baseline via torchrun."""
    ...
    story_metadata = {
        ...
        "optimization_mechanism": "all_gather_full_sequence_after_tp_all_reduce",
    }
    ...
    def get_torchrun_spec(self, config: Optional[BenchmarkConfig] = None) -> TorchrunLaunchSpec:
        ...
        return TorchrunLaunchSpec(
            ...
            script_args=[],
            multi_gpu_required=True,
            name="baseline_sequence_parallel_multigpu",
            config_arg_map={"iterations": "--iters", "warmup": "--warmup"},
        )


def get_benchmark() -> BaseBenchmark:
    return BaselineSequenceParallelMultigpuBenchmark()
