"""Baseline Medusa/EAGLE family benchmark (target-only greedy decode)."""

from __future__ import annotations

from core.harness.benchmark_harness import BaseBenchmark

from ch15.medusa_eagle_speculative_benchmarks import (
    MedusaEagleSpeculativeBenchmark,
    attach_benchmark_metadata,
)


BASELINE_STORY_METADATA = {
    "pair_role": "canonical",
    "variant_role": "baseline",
    "chapter_alignment": "native",
    "chapter_native_exemplar": True,
    "comparison_axis": "target_only_greedy_decode_vs_explicit_medusa_eagle_speculative_profiles",
    "optimization_mechanism": "target_only_greedy_decode_without_draft_acceptance",
}


def get_benchmark() -> BaseBenchmark:
    bench = MedusaEagleSpeculativeBenchmark(
        variant="baseline",
        label="baseline_medusa_eagle_speculative",
    )
    bench.story_metadata = dict(BASELINE_STORY_METADATA)
    return attach_benchmark_metadata(bench, __file__)
