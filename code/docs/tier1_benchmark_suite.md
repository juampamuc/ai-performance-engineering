...
That command reads the suite definition from [`configs/benchmark_suites/tier1.yaml`](/home/cfregly/ai-performance-engineering/code/configs/benchmark_suites/tier1.yaml) and writes a canonical history package under `artifacts/history/tier1/<run_id>/`.

The recurring CI entrypoint is [`tier1-nightly.yml`](/home/cfregly/ai-performance-engineering/code/.github/workflows/tier1-nightly.yml). It defaults to `["self-hosted","gpu"]` runner labels, but you can override that with the repository variable `TIER1_RUNNER_LABELS_JSON`.

## Latest Snapshot
Latest recorded canonical history package:

- Run id: `20260309_140000_tier1_recheck_refresh2`
- Succeeded targets: `6/6`
- Representative speedup: `7.38x` geomean
- Median speedup: `8.79x`
- Arithmetic average speedup: `21.97x`
- Largest measured win: `ch04:gradient_fusion` at `95.35x`
- Latest fully green rerun followed the harness cleanup for custom-metric capture, profiler-child teardown, and regression recheck handling.

Primary artifacts:
- `artifacts/history/tier1/20260309_140000_tier1_recheck_refresh2/summary.json`
- `artifacts/history/tier1/20260309_140000_tier1_recheck_refresh2/regression_summary.md`
- `artifacts/history/tier1/20260309_140000_tier1_recheck_refresh2/trend_snapshot.json`

The dashboard now exposes this same artifact package directly on the Tier-1 page; it reads the canonical history index instead of inventing a second reporting path.
...
- [tier1.py](/home/cfregly/ai-performance-engineering/code/core/benchmark/suites/tier1.py)
- [history_index.py](/home/cfregly/ai-performance-engineering/code/core/analysis/history_index.py)
- [regressions.py](/home/cfregly/ai-performance-engineering/code/core/analysis/regressions.py)
- [trends.py](/home/cfregly/ai-performance-engineering/code/core/analysis/trends.py)
...
- The regression layer treats speedup changes as significant only when they clear both the relative threshold and a small absolute optimized-time floor, so very small kernels do not produce noisy false regressions from sub-`0.05 ms` drift alone.
...
- [performance_repo_roadmap.md](/home/cfregly/ai-performance-engineering/code/docs/performance_repo_roadmap.md)
- [benchmark_harness_guide.md](/home/cfregly/ai-performance-engineering/code/docs/benchmark_harness_guide.md)
