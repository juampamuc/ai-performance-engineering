...
- `launch_bounds` and `launch_bounds_cuda` are small-effect teaching cases for the `__launch_bounds__` annotation. They remain useful for profiler study and code reading, but they now stay on local expectation-backed contracts instead of being skipped as informational surfaces.
